        ...
        while True:
            ...
            if len(self.proc_queue) > 0:
                ...
                # if the destination node is this current node
                    ...
                    pkt = self.proc_queue.pop()
                    if pkt[Globals.CPU_BURST] - (self.quantum * pkt[Globals.NO_ROUNDS]) > 1:
                        ...
                        yield self.env.timeout(pkt[Globals.CPU_BURST] - (self.quantum * pkt[Globals.NO_ROUNDS]))
                        pkt[Globals.NO_ROUNDS] += 1
                        ...
                        if self.verbose >= Globals.VERB_LO:
                            Utils.report(self.env.now, self.name, pkt, self.proc_queue,
                                         ...
                                         self.verbose)
                        if pkt[Globals.DEST_TIME_STAMP] - pkt[Globals.TIME_STAMP] > Globals.DICTIONARY[pkt[Globals.ID]]["servicetime"]:
                            Globals.DICTIONARY[pkt[Globals.ID]]["servicetime"] = pkt[Globals.DEST_TIME_STAMP] - pkt[Globals.TIME_STAMP]
                            Globals.DICTIONARY[pkt[Globals.ID]]["timestamp"] = Globals.DICTIONARY[pkt[Globals.ID]]["timestamp"] -1
        ...
        if int(self.name) in range(1, 21):
            ...
            while True:
                ...
                while i < p_size:
                    ...
                    pkt = self.make_pkt(pkttype, destination, id)
                    self.proc_queue.appendleft(pkt)
                    self.generated.append([self.env.now, pkt])
                    i +=1

                Globals.DICTIONARY[id] =  {"p_size" : p_size, "servicetime" : 0, "timestamp": p_size}
                # report as per verbose level
                if self.verbose > Globals.VERB_NO:
                    Utils.report(self.env.now, self.name, pkt, self.proc_queue, inspect.currentframe().f_code.co_name,
                                 self.verbose)

    def make_pkt(self, pkt_type, dest_node, id):
