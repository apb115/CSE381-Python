...
class Node:
    ...
    def forward_process(self):
        ...
        while True:
            ...
            # Sort the packets in the queues of the servers 25 - 28 based on their priorities XXXXXXXXXXXXXX
            if int(self.name) in [25,26,27,28]:
    ...
    def pkt_gen_process(self):
        ...
        if int(self.name) in range(1, 21):
            ...
            while True:
                ...
                reason = ["mail", "media", "storage", "navigation"]
                pkt_type = reason[random.randrange(0, 3, 1)]

                # Specify the server (dest_node) on which the packet should be scheduled and save it in 'dest_node' XXXXXXXXXXX
                if pkt_type == "mail": dest_node = str(25)
                if pkt_type == "media": dest_node = str(26)
                if pkt_type == "storage": dest_node = str(27)
                if pkt_type == "navigation": dest_node = str(28)
    ...
    def make_pkt(self, pkt_type, dest_node):
        ...
        pkt = {
            ...
            # Add here a new packet attribute called 'PRIORITY' contains value returned from the method run() in the module Priority.py
            Globals.PRIORITY: Priority.run(), # XXXXXXXXXXX
