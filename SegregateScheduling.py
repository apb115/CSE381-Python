...
class Node:
    ...
    def pkt_gen_process(self):
        ...
        if int(self.name) in [1,2,3,4,5,6,7,8]:
            reason = ["email", "media", "storage", "navigation"]

            while True:

                # Generate random type
                cur_type = reason[random.randrange(0, 3, 1)]
                ...
                # get the source node
                source = int(self.name)

                # get the destination node
                #dest_node = str(random.choice([13,14]))
                if cur_type == "email": dest_node = str(13)
                if cur_type == "media": dest_node = str(14)
                if cur_type == "storage": dest_node = str(15)
                if cur_type == "navigation": dest_node = str(16)

                # generate the packet
                pkt = self.make_pkt(dest_node, cur_type)
    ...
    def make_pkt(self, dest_node, cur_type):
